...
|-------------|---------|------------------------------------------------------------------------------------------------------------------------|
| `inventory` | no      | Enables [`set_all_from_env()`], error types, and the `inventory` registry. Intended for binary crates that don't use clap. |
| `clap`      | no      | Implies `inventory`. Enables [`EnvArgs`], [`args()`], and [`set_all_from_matches()`]. Adds a dependency on `clap`.            |
